...
export function notesScreenHtml(entries) {
  const body =
    entries.length === 0
      ? `<p class="notes-screen__empty">Нотаток поки немає. Відкрийте будь-яке завдання, розгорніть блок «Моя нотатка» — і запишіть свій висновок або альтернативне рішення.</p>`
