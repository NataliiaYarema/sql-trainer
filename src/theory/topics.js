...
const topics = [
...

  {
    ...
    keywords: ['CASE', 'COALESCE', 'NULLIF', 'UNION', 'INTERSECT', 'EXCEPT'],
    summary:
      'CASE дозволяє повертати різні значення залежно від умови. Він перевіряє умови ' +
      'по черзі й повертає результат для першої умови, яка виконується. Якщо жодна умова ' +
      'не виконується, використовується ELSE. Якщо ELSE не вказаний, результатом буде NULL. ' +
      'END позначає кінець конструкції CASE.',
    summaryBlocks: [
      'COALESCE використовується, коли потрібно замінити NULL іншим значенням. Він повертає ' +
        'перше значення зі списку, яке не є NULL. Це зручно, наприклад, коли замість ' +
        'відсутнього значення потрібно показати зрозумілий текст або використати запасне значення.',
      'NULLIF працює у зворотному напрямку: перетворює конкретне значення на NULL, якщо воно ' +
        'збігається із заданим. Це часто використовують, щоб перетворити нуль на NULL перед ' +
        'діленням і уникнути ділення на нуль.',
      'UNION, UNION ALL, INTERSECT та EXCEPT дозволяють порівнювати або обʼєднувати ' +
        'результати двох запитів.',
      'На відміну від JOIN, який додає колонки з іншої таблиці, операції з множинами ' +
        'працюють із рядками: результати двох запитів обʼєднуються один під одним.',
      [
        'UNION — обʼєднує результати та прибирає дублікати.',
        'UNION ALL — обʼєднує результати, зберігаючи дублікати.',
        'INTERSECT — залишає рядки, які є в обох результатах.',
        'EXCEPT — залишає рядки з першого результату, яких немає в другому.',
      ],
      'Для всіх чотирьох операцій обидва запити повинні повертати однакову кількість ' +
        'колонок, а відповідні колонки — мати сумісні типи даних.',
    ],
...

  {
    ...
    summaryBlocks: [
      ...
      'Головне правило аналітика: написати SQL — лише частина роботи. Найважливіше — ' +
        'правильно зрозуміти бізнес-запит і порахувати саме те, що потрібно бізнесу.',
    ],
